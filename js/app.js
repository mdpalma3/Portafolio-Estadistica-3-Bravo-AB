  ...
  lista.innerHTML = archivos
    .map((a, i) => {
      ...
      <div class="item-archivo">
        ...
        <button title="Vista previa" onclick='abrirVistaPrevia(${JSON.stringify(a.nombre)}, ${JSON.stringify(a.tipo)})'><i class="fa-solid fa-eye"></i></button>
        <button title="Descargar" onclick="alert('Descarga simulada de: ${a.nombre.replace(/'/g, "\\'")}')"><i class="fa-solid fa-download"></i></button>
...
function abrirVistaPrevia(nombre, tipo) {
  ...
  document.getElementById("modal-cuerpo-contenido").innerHTML = `
    <div class="vista-doc">
      ...
      <p style="font-size:12.5px;">Vista previa de demostración — en la versión final aquí se incrusta el visor real del documento.</p>
    </div>
    <div style="display:flex;gap:10px;justify-content:flex-end;">
      <button class="btn btn-fantasma" onclick="cerrarModal()">Cerrar</button>
      <button class="btn btn-dorado" onclick="alert('Descarga simulada de: ${nombre.replace(/'/g, "\\'")}')"><i class="fa-solid fa-download"></i> Descargar</button>
    </div>
  `;
